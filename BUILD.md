...
```bash
git clone https://github.com/kylebegeman/murmur.git
cd murmur
```
...
The raw binary (`src-tauri/target/release/handy` — the crate binary keeps the upstream `handy` name for now) cannot run standalone — it needs Tauri resource files (tray icons, sounds, VAD model) to be co-located at the expected path.
...
After subsequent rebuilds, copy the binary and any refreshed runtime libraries (note the cargo binary is still named `handy`; the installed one is `murmur`):

```bash
sudo cp src-tauri/target/release/handy /usr/bin/murmur
...
this because it builds from a short root such as `D:\a\murmur`.)
...
Alternatively, clone the repo to a short root (e.g. `C:\murmur`).
