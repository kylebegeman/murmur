...
/// The base code Murmur matches a language *intent* on: a tag's primary subtag,
...
/// Bridges hf-hub's async download progress to Murmur's `model-download-progress`
...
impl ModelManager {
    ...
    /// models dropped in or downloaded outside Murmur show up without a restart.
    ...
    fn discover_custom_transcribe_models(
        ...
        for entry in fs::read_dir(models_dir)? {
            ...
            // Murmur-downloaded one. Legacy `.bin` files have no GGUF header, so
    ...
    /// shared Hugging Face cache, so models downloaded by Murmur (or any other
