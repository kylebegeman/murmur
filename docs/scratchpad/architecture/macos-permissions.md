...
| `src-tauri/tauri.conf.json`                             | `identifier: com.kylebegeman.murmur`, `productName: Murmur`, `app.macOSPrivateApi: true` (needed for transparent overlay), `app.windows: []` (all windows created programmatically), `bundle.macOS.hardenedRuntime: true`, `signingIdentity: "-"` (ad-hoc!), updater plugin config pointing at kylebegeman/murmur GitHub releases (pubkey empty — fails closed until a Murmur signing keypair exists; see `docs/product/rebrand-decisions.md`). |
